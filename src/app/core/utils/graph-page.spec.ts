...
function commit(sha: string, lane: number, row: number): GraphCommit {
  return {
    sha,
    lane,
    parent_shas: [],
...
describe('appendGraphPage', () => {
  ...
  it('does not mutate the pages it merges', () => {
    const first = page([commit('a', 0, 0)], 1);
    const second = page([commit('b', 0, 1)], 1);
