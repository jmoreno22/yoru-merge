...
| Compact density | Summary line and files header at `--panel-head-h` and file rows at `--file-row-h` as `computeMetrics` derives them for the active font — the three values each token takes are in [`reference-figures.md`](./reference-figures.md) §Density tokens — panel padding `--panel-pad`; type scale unchanged | AC-20 | <!-- figures replaced by a pointer 2026-09-13 (T77, review round 19 R19-F5, per owner decision D2): this row gave the compact pair as «26 and 24 at the default 13 px». Both were correct, and both are `computeMetrics` outputs, which owner decision D3 forbids a live artefact from writing. The generated table carries all three densities and is byte-compared by `pnpm check:figures`, so it cannot go stale -->
