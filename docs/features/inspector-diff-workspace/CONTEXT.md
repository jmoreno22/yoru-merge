...
- commit file list — the list of files changed by the selected commit, shown in the inspector between the commit header and the diff viewer, as a tree or a flat list; in this feature it is compact, collapsible and bounded in height. NOT the working-changes lists (staged / unstaged files in the Changes centre view).
...
- diff viewer — the diff pane inside the inspector, with its layout (unified / side-by-side), whitespace, wrap, context-lines and hunk-navigation controls. NOT the diff workspace.
