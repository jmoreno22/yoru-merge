...
| T26 | Record the task-boundary widenings and refresh the screenshots | docs | Jhoan Moreno | S | T22, T23 | done |
...
**T26 — closed 2026-09-13.** The boundary notes were committed long ago; the three screenshots (`docs/screenshots/history-dark.png` and `changes-light.png` recaptured, new `diff-workspace-dark.png` with the workspace open from History, plus its README line) were captured from `pnpm tauri dev` with this repository open, at 1296 x 809 to match the existing set, once the owner gave the dev-build window focus. The blocker was never the visual check itself — it was that the capture needed a focused window and the right binary: the first attempts drove the **installed** 1.0.6, which does not carry the reversal.
