...
**Intent.** Make the commit file list the dominant surface of the inspector whenever a commit is selected, and let the developer hand any single file's diff the full centre width and come back in one gesture each way, from both the History and the Changes view. The commit header shrinks to an orientation aid that yields height before the list does, and the commit diff leaves the inspector entirely — it is read at centre width in the diff workspace, which is a state of the centre view (not a route, window, tab or overlay) that restores the exact previous selection, scroll and focus on close. Every existing layout mode keeps working unchanged, so the redesign is additive for existing habits. <!-- amended 2026-09-08 (owner, review round 9 R9-S1-F3): the Intent still described the pre-reversal column and contradicted quality goal 1 four lines below -->

**Top-3 quality goals (1-liners; full scenarios in §10):**

1. **File-list dominance in the inspector** — the commit file list receives ≥ 50 % of the inspector height with the header expanded and ≥ 75 % collapsed (inspector right, no stacked panels), and never less at the bottom or with stacked panels, **except where the header-cap guard costs the list its ratio — a remainder under twice `--panel-head-h` with the header expanded, or under four times it collapsed: there the cap is lifted to a full panel head and the ratio yields, because a header shorter than its own head disappears behind its own scrollbar (AC-03)**. The commit diff is not read in the inspector at all; it is read at centre width in the diff workspace. <!-- amended 2026-09-07 (owner); carve-out added 2026-09-09 (owner, review round 14 R14-S1-F1): rounds 12 and 13 carved the cross-placement guarantee out of AC-18, spec §6 row 3, §10 QG-1, §6 F2, the test plan, the screen manifest and the UX flows, and this goal — the §1 one-liner of the very quality goal whose full scenario is §10 QG-1 — was in none of those waves, because it phrases the guarantee with «never less» rather than «share» and every sweep enumerated phrasings of «share». It is false at the bottom’s 220 px minimum with both panels stacked and the header collapsed: 0.690909 comfortable at the default 13 px against 0.750 on the right. Wording byte-identical to AC-18’s. Band restated 2026-09-09 (owner, review round 15 R15-S1-F1 / R15-S1-F2): the carve-out named two fixed pixel pairs taken from the spec fixtures, but `--panel-head-h` is computed per density and font (`appearance-metrics.ts:78-98`), so over the 21 reachable pairs the collapsed band runs 96…192 px rather than 136 / 120 and 688 reachable columns missed their floor outside it — worst 0.647059 at relaxed / 17 px, column 272. The band is now the guard’s own closed form, exact at 201 474 measured points across all 21 pairs and both collapse states. The compact figure this marker used to give, 0.727273, came from the unreachable fixture `COMPACT_TOKENS {30, 30}`: real compact at 13 px is 24 / 26 and MEETS the floor in that configuration (0.754545). The reachable compact counter-example is 0.741667 at compact / 17 px, column 240 -->
...
- Tailwind CSS 4.3.3. Every fixed dimension comes from the density tokens `AppearanceService` writes on `<html>` (`--row-h`, `--file-row-h`, `--panel-head-h`, `--panel-pad`, …); `--file-row-h` and the CDK virtual-scroll `itemSize` are the **same** value — every virtual list binds `[itemSize]="rowHeight()"` (`commit-inspector.html`, `changes-list.html`, `blame-viewer.html`, `file-history-panel.html`, `commit-list.html`) and the token is written from the same `computeMetrics` output — so `--file-row-h` must not be overridden in CSS: an override moves the rows without moving the `itemSize` the viewport scrolls by. <!-- corrected 2026-09-10 (T63, review round 16 R16-L-F1): this constraint named a constant `FILE_ROW_HEIGHT` that does not exist in the repo (`grep -rn "FILE_ROW_HEIGHT" src` is empty), which invited a reader to discard the whole sentence including the half that is real and that `styles.css:289-295` enforces. T59 recorded the same fact one file over at `screens.md`'s Density bullet in the round-15 wave and did not carry it here --> Z-index only from the four `--z-*` tokens (`DESIGN.md` §Z-layers).
...
- Minimum window 960 × 640; the three shipped densities (compact / comfortable / relaxed, `preferences-schema.ts:214`), both placements (inspector right / bottom), both themes. <!-- widened 2026-09-09 (owner, review round 15 O2): the constraint said «both densities» while the app has shipped three since 1.0.5, and the third is where the header-cap guard costs the list its ratio over the widest band -->
...
| `preferences.json` (Tauri plugin-store) | Datastore (internal, on disk) | Read on boot, written debounced 500 ms. Gains three durable preferences: the remembered collapsed states of the commit header and the commit file list (AC-02, AC-05) and the open-behaviour choice `commitFileClickOpensWorkspace` (AC-22) <!-- amended 2026-09-08 (owner, review round 10 R10-S1-F6): the row that enumerates what the datastore gains still named two -->; already holds the diff viewer's layout / whitespace / wrap / context preferences the diff workspace shares (AC-06) |
...
3. **One diff viewer instance, re-hosted into the centre when the workspace opens** (ADR-0003). The existing `DiffViewer` (options bar + `DiffView`) stays the only renderer of a diff. Its live host element moves from the inspector slot into the workspace slot with a CDK `DomPortal` (moves the DOM, no destroy / recreate) and back on close, so opening never re-fetches (`repo.diffText` untouched) nor re-renders a 2 000-line non-virtualised diff (NFR first paint ≤ 150 ms), and the layout / whitespace / wrap / context controls are literally the same widgets bound to the same preferences (AC-06). The one control the workspace withholds is the per-file collapse chevron: it shows a single file at full width, and collapsing it would leave the centre empty <!-- amended 2026-09-08 (owner, review round 9 R9-S1-F10): the gate shipped in `2049df4`; AC-06 now states it -->. The inspector's diff slot is the element's parking home: it collapses to zero height while the element is away, and in the History view it stays collapsed permanently, because there the inspector shows no diff at all (AC-06). In the Changes view it is a real slot again, so the working tree keeps its inline viewer; hunk / line staging keeps working because the diff source is unchanged; <!-- amended 2026-09-07 (owner) --> the workspace itself is a thin header (file path, source, previous / next, Close) around the portal outlet. Rejected: a second `<app-diff-viewer>` in the centre (full re-render of an unvirtualised diff on every open, duplicated line-selection state).

4. **Size the inspector blocks with an explicit, testable layout policy** (ADR-0004). The inspector column stops being `flex-[2] / flex-[3]`: the commit header is content-sized (body clamped to 4 lines, floor 1), the commit file list takes the remainder with a floor of 50 % of the inspector (2 rows min, exact row count when fewer, header-only when collapsed) and the History inspector has no diff slot to arbitrate against. Because AC-03 still requires a *sequential* yield — now the body clamp shrinks first and the header then scrolls inside its cap, so the list is the protected block rather than the diff — the clamp lines and the header cap come from a pure-TS `inspector-layout.ts` function <!-- amended 2026-09-07 (owner) --> of (available height, file count, density tokens, collapsed states) fed by a `ResizeObserver` and applied as CSS variables — not from flex-shrink weights, which shrink proportionally and cannot be unit-tested. Stacked blame / file history keep their current share (AC-19); the bottom placement runs the same policy (AC-18). Rejected: a CSS-only flex / `min-height` / `line-clamp` arrangement (cannot express the sequential order; floors untestable without a browser tier).

5. **Remembered states are ordinary durable preferences** (inline — repo convention, no alternative). `commitHeaderCollapsed`, `commitFileListCollapsed` and `commitFileClickOpensWorkspace` join `DurablePreferences` in `preferences-schema.ts` (plugin-store `preferences.json`, debounced 500 ms) next to the existing diff options. The third is the open-behaviour choice of AC-22, boolean, defaulting to **on** (a single click opens the diff workspace); a stored non-boolean falls back to the default. <!-- amended 2026-09-08 (owner, review round 9 R9-S1-F1): AC-22's preference shipped in `2049df4` and was named in no SAD section --> The inspector reads them synchronously at render; since it only renders after a repository is open and a commit is selected, the store has resolved long before its first paint (NFR 0 px layout shift), so no localStorage mirror is needed.
...
- **Who shows the diff.** Only `DiffViewer`. The workspace component hosts its live element through a `CdkPortalOutlet` (ADR-0003); the inspector's diff slot is the element's parking home and collapses while it is away — permanently so in History, where the inspector shows no diff. <!-- amended 2026-09-07 (owner) --> Staging controls stay gated by `diffSource` (commit → none; working tree → hunk / line stage and unstage), so AC-14 needs no new rule.
...
- **Who sizes the inspector.** `CommitInspector` observes its own height and asks `inspector-layout.ts` for the body clamp lines and the header cap (ADR-0004); the released height flows to the commit file list, which is the only growing child in History. <!-- amended 2026-09-07 (owner) -->
...
│   │                                    → { listRows, clampLines, headerMaxH } with the AC-03 floors (+ .spec.ts)
...
│   └── preferences-schema.ts            + commitHeaderCollapsed, commitFileListCollapsed,
│                                        commitFileClickOpensWorkspace (durable, preferences.json)
├── features/
...
└── shared/components/main-content/      centre: `@if (workspace.open())` before `@switch (view())`;
                                         inspector column: diff slot = portal source; flex-1 in Changes, h-0 in History
...
C4Container
    ...
    ContainerDb(prefs, "preferences.json", "Tauri plugin-store", "Durable UI preferences, now including the three this feature adds")
...
sequenceDiagram
    ...
    Dev->>Inspector: activate a file row
    alt commitFileClickOpensWorkspace is on (the default)
        Note over Inspector: a single click opens; double-click, the open-large control and the shortcut also open
    else the preference is off
        Note over Inspector: a single click only makes the row active; opening needs double-click, the open-large control or the shortcut
    end
...
sequenceDiagram
    ...
    UI->>S: read the remembered states of the header, the file list and the open behaviour
    S->>D: read commitHeaderCollapsed, commitFileListCollapsed and commitFileClickOpensWorkspace
    D-->>S: remembered states (already resolved, read synchronously)
    S-->>UI: collapsed states and the open-behaviour mode
    UI->>S: size the inspector (available height, displayed row count, density tokens, collapsed states)
    alt no rows to draw (no changed files, or a filter that matches none)
        S-->>UI: list header with the count plus one explanatory line; the list claims a bare head and no share
    else fewer rows than the column fits
        S-->>UI: exactly as many rows as there are, no reserved empty space
    else more rows than the column fits
        S-->>UI: every row the column fits, with the list's own scroll and the total count in the header
    end
    alt the file list would fall below its share (minimum window, long body, many files)
        S-->>UI: shrink the body clamp first, floor one line plus "show more"
        opt still below the share
            S-->>UI: cap the expanded header and let it scroll inside the cap; the list never gives up a row
        end
    else the height fits
        S-->>UI: body clamped to four lines plus "show more" (no clamp when the body is shorter)
    end
    UI-->>U: header expanded (or in its remembered collapsed state), the file list holding the rest of the column
    Note over UI,S: no persist in this flow, the remembered states are only read and the layout is derived
    Note over U,UI: Postcondition: header and file list paint directly in their remembered state (0 px shift), the commit file list keeps at least half the inspector and no diff is shown in this column
```
<!-- amended 2026-09-08 (owner, review round 9 R9-S1-F2): the file-count branch capped at six rows and the yield branch shrank the list before the clamp — both are what the 2026-09-07 reversal inverted; the diff viewer left this column entirely. AC-22's read of `commitFileClickOpensWorkspace` was added to the first-paint step (R9-S1-F1). -->

The developer selects a commit. The UI asks the service for the commit details and its changed files, which the service reads from the repository; the service also reads the three remembered states from the preferences store synchronously, so the first paint is already correct. The service then runs the height policy on the available inspector height, the number of rows the list will draw, the density tokens and the collapsed states. The file list branch sizes the list (nothing to draw: a bare header with the count and one explanatory line, claiming no share; fewer rows than fit: exact rows; more: every row that fits, with scroll and the count — there is no upper bound). The height branch handles the squeezed case (the body clamp shrinks first to a floor of one line, then the expanded header is capped and scrolls inside that cap) and otherwise clamps the body to four lines; the list never gives up a row to the header. Nothing is persisted here.
...
sequenceDiagram
    ...
    Note over U,UI: Precondition: a commit is selected, the inspector shows the commit header and the commit file list (flow F1)
    ...
    alt header collapsed
        S-->>UI: one summary line (subject, author, short sha) with the six actions inline, released height to the commit file list
        Note over UI: inspector on the right without stacked panels, the file list now takes at least three quarters
    else file list collapsed
        S-->>UI: list header with the file count only, the released height left empty, active file unchanged
        Note over UI: a collapsed list claims no share, and the stacked panels hold fixed shares, so nothing grows into it (AC-05, AC-19)
    end
    alt blame or file history stacked under the commit file list
        S-->>UI: stacked panels keep their fixed share, only the commit file list grows
    else inspector placed at the bottom
        S-->>UI: same policy over the shorter remainder, released height to the commit file list, the list keeping at least the share it has on the right, except where the header-cap guard costs the list its ratio (AC-18)
...
The developer collapses the header (control or shortcut) or the file list. The UI hands the new collapsed state to the service, which writes it to the preferences store (a debounced durable preference, no new entity) and re-runs the height policy. Header collapsed: one summary line with the six actions inline, released height to the commit file list (three quarters on the right without stacked panels). File list collapsed: header with the count only, active file unchanged, and the released height left empty — the list claims no share and the stacked panels hold fixed ones, so there is nothing to grow into it. The placement branch then says what happens to the height the header releases: with blame or file history stacked they keep their fixed share and only the commit file list grows; at the bottom the same policy runs over the shorter remainder and the list keeps at least the share it has on the right, **except where the header-cap guard costs the list its ratio — a remainder under twice `--panel-head-h` with the header expanded, or under four times it collapsed: there the cap is lifted to a full panel head and the ratio yields, because a header shorter than its own head disappears behind its own scrollbar (AC-03)**; on the right without stacked panels the measured targets apply. Expanding again clears the state, writes it, and restores the previous expanded layout. The state survives a restart because F1 reads it at first paint. <!-- amended 2026-09-08 (owner, review round 9 R9-S1-F2, R9-S1-F8, R9-S2-F6b): the released height went to a diff viewer this column no longer has, the collapsed-list branch promised it to a stacked panel that cannot take it, and the bottom placement promised a height rather than a share; carve-out added 2026-09-08 (owner, review round 13 R13-S1-F1): round 12 carved the cross-placement guarantee out of AC-18, spec §6 row 3, §10 QG-1 and the test plan, and neither the F2 message label nor this paragraph was among them, so §6 still promised what the policy misses at the bottom’s 220 px minimum with both panels stacked and the header collapsed (0.691 comfortable / 0.727 compact against 0.750). The paragraph carries the canonical sentence byte-identical to AC-18’s; the diagram label carries the pointer, because a 313-character sentence inside a mermaid message is unreadable. The paragraph is a FIFTH site the round-13 reviewers did not enumerate, found by sweeping every carrier of the claim rather than the four they named. Band restated 2026-09-09 (owner, review round 15 R15-S1-F1): the two fixed pixel pairs were derived from the spec fixtures, so the exception was narrower than the guard over 12 of the 21 reachable density × font pairs; it is now the guard’s own closed form, exact at 201 474 measured points -->

### Flow F9 — Switch what a single click on a file row does, and remember it

(US-03 — AC-22.) Added 2026-09-08 (owner, review round 9 R9-S1-F1): AC-22 entered §5 with the 2026-09-07 reversal and reached no §6 flow. Same generic vocabulary as F1 / F2.
...
sequenceDiagram
    ...
    Note over U,UI: Precondition: a commit is selected and the commit file list is visible (flow F1), which has already read the current mode
    U->>UI: use the open-behaviour control in the file list header
    Note over UI: the control names the mode that is LIVE, not the one it would switch to
    UI->>S: set the open behaviour to the other mode
    S->>D: write commitFileClickOpensWorkspace
    Note over S,D: a durable preference like the two collapsed states (debounced write, no new entity)
    D-->>S: ack
    S-->>UI: the live mode, echoed back to the control
    alt the preference is on
        UI-->>U: a single click on a file row opens the diff workspace
    else the preference is off
        UI-->>U: a single click only makes the row active
    end
    Note over U,UI: Neither mode changes the other three gestures: double-click, the open-large control and the shortcut open the workspace in both
    Note over U,UI: Postcondition: the mode survives an app restart and paints directly, like the header and file-list collapse states (flow F1 reads it)
```

The developer uses the open-behaviour control that sits in the file list header beside the tree and flat toggles. The UI hands the new mode to the service, which writes it to the preferences store and echoes back the mode that is now live — the control always shows the live mode, never the one it would switch to. With the preference on, a single click on a file row opens the diff workspace (critical flow 1); with it off, a single click only makes the row active. The three explicit gestures are unaffected in both modes. The mode survives a restart because F1 reads it at first paint, so the list never paints with the wrong click behaviour.
...
Written by `sequences` (2026-09-03). «Critical flow 1 / 2» are the two flows seeded by `design` above; F1–F9 and the cross-cutting Esc flow were added here (F9 on 2026-09-08 for AC-22, and it is placed after F2 rather than after F8 — the coverage table below is the order of record). Every §4 user story maps to at least one flow and every §5 AC to a flow, an `alt` / `else` branch or an explicit non-runtime N/A.

| User story | Realised by |
|---|---|
| US-01 | F1, F2 |
| US-02 | F1, F2 |
| US-03 | Critical flow 1, F4, F7, F9 | <!-- F9 added 2026-09-08 (owner, review round 11 O8): the AC-22 row already named it, only this US row was short -->
...
| AC-03 | F1 | «would fall below its share» branch: the clamp floors at one line, then the expanded header scrolls inside its cap; the list keeps its two-row floor <!-- amended 2026-09-08 (owner, R9-S1-F2): the yield order was stated the pre-reversal way round --> |
| AC-04 | F1 | row-count branch: nothing to draw / fewer rows than fit / more rows than fit (no upper bound) <!-- amended 2026-09-08 (owner, R9-S1-F2) --> |
...
| AC-22 | F9 (+ critical flow 1's opening `alt`) | the toggle step with its persist note, the live-mode echo, and the two click branches; critical flow 1 shows the same two modes at the point of the gesture <!-- added 2026-09-08 (owner, R9-S1-F1) --> |

### Notes for `design` / `data-model` (flags, not decisions)

- **Participant vocabulary.** F1–F9 use the generic `sequences` vocabulary (`user`, `ui`, `service`, `data-store` = the preferences store, `external-system` = the local Git repository and the OS file-system notifications). The two critical flows seeded by `design` keep the §5 container names; they were left untouched (additive rule). Angle brackets were dropped from the generic names because Mermaid strips them as HTML.
- **Persist notes for `data-model`.** Only F2 and F9 write: `commitHeaderCollapsed`, `commitFileListCollapsed` and `commitFileClickOpensWorkspace` as durable preferences (§4 pillar 5). <!-- amended 2026-09-08 (owner, R9-S1-F1): the third preference and the flow that writes it --> No new entity, column or index in any flow; the diff workspace state (F4, F5, F7, F8) is never persisted. `data-model`'s no-schema-change condition holds.
...
- Release-time measurements (spec §6): the commit file list's protected share of the inspector at 960 × 640 and 1280 × 800 in the three shipped densities; <!-- amended 2026-09-08 (owner, review round 9 R9-S1-F3): the measurement named the diff viewer the reversal removed from this column --> performance-panel timing of the workspace open on a 2 000-line diff; the six-layer Esc keyboard matrix; layout-shift on launch in both remembered states; the release checklist over both themes × the three shipped densities × both placements.
- Alerts / tracing: none.

**Scaling thresholds:**
- Not applicable to a desktop app. The two axes that grow are the commit file count — the list draws every row the column fits and virtualises the rest, so a 30- or 3 000-file commit costs the same inspector height and the same render work (`cdk-virtual-scroll-viewport`) <!-- amended 2026-09-08 (owner, review round 9 R9-S1-F3): the 6-row cap went with the diff slot; the bound is now the column height, and virtualisation is what keeps the cost flat --> — and the diff size, budgeted at the spec's 2 000-line measurement; `DiffView` is unvirtualised and the workspace neither improves nor worsens that (ADR-0003) — see §11.
...
**QG-1. File-list dominance in the inspector** <!-- heading amended 2026-09-08 (owner, review round 9 O5): only the scenario body was carried through on 2026-09-07 -->
...
- **Then:** the commit file list's share of the inspector height is ≥ 50 % at every window size from 960 × 640 upward (expanded) and ≥ 75 % (collapsed); at the bottom or with stacked panels its share is ≥ the share it keeps with the inspector on the right, measured on the remainder the stacked panels leave, **except where the header-cap guard costs the list its ratio — a remainder under twice `--panel-head-h` with the header expanded, or under four times it collapsed: there the cap is lifted to a full panel head and the ratio yields, because a header shorter than its own head disappears behind its own scrollbar (AC-03)**; at the minimum window the body clamp floors at 1 line and the header scrolls inside its cap before the list ever drops below 2 rows (AC-03). The share is not measured when the list has no row to draw — it keeps only its head by design (AC-04, AC-05). <!-- amended 2026-09-07 (owner); amended 2026-09-08 (owner, review round 10 R10-S1-F2 / R10-S1-F1): «height» was the height/share mixture R9-S2-F6b resolved in favour of share, and the empty-list carve-out was missing; carve-out for the hard floors added 2026-09-08 (owner, review round 11 R11-S2-F4): the scenario asserted the cross-placement share with no exception, and it is missed at the bottom's 220 px minimum with both panels stacked and the header collapsed; carve-out corrected 2026-09-08 (owner, review round 12 R12-L-F1 / R12-S1-F1 / R12-S2-F4): the round-11 wording dropped both named mechanisms here, and the one it named elsewhere was wrong — the list's two-row floor causes none of the 224 misses, and a single 136 / 120 band waived the expanded guarantee over a remainder of 68…135 px where the policy meets it. Now byte-identical to AC-18's and §6 row 3's sentence. Band restated 2026-09-09 (owner, review round 15 R15-S1-F1): the two fixed pixel pairs were derived from the spec fixtures, so the exception was narrower than the guard over 12 of the 21 reachable density × font pairs; it is now the guard’s own closed form, exact at 201 474 measured points -->
- **How verify:** the spec §6 measurement — the height the commit header may not cross, `(inspector height − headerMaxH) / inspector height` — read at 960 × 640 and 1280 × 800 in the three shipped densities, and applied to the remainder after the stacked panels' fixed shares for the bottom and stacked configurations; `inspector-layout.spec.ts` asserting the yield order and the floors for those configurations. <!-- amended 2026-09-08 (owner, review round 10 R10-S1-F2): this scenario still prescribed the two methods round 9 closed — an element-height measurement in the built app (R9-S2-F6a), which cannot be right for a list that stops at its file count, and a side-by-side against the 1.0.5 build (R9-S1-F7), the baseline amended AC-18 replaced -->
...
- **When:** the redesigned inspector renders in each existing mode (both themes × the three shipped densities × both placements) and on launch with a remembered collapsed or expanded state.
...
- **How verify:** release checklist over both themes × the three densities × both placements; layout-shift measurement on launch in both states; keyboard-only accessibility pass; `pnpm build` with `strictTemplates` as the component-contract gate.
...
| Height policy with blame and file history both stacked at 960 × 640: fixed shares plus the file list's 50 % floor may not fit | Low | The policy treats stacked shares as fixed and applies floors to the remainder (AC-19 measures the target without stacked panels); stacked case in the release checklist | Owner |
| Product: a long commit file list pushes the commit header into its cap on a short column | Low | The clamp yields first and the header scrolls inside the cap rather than the list losing rows (AC-03); collapsed / expanded state remembered; previous / next in the workspace replaces list scrolling; review the spec §7 KPIs after the first release | Owner | <!-- amended 2026-09-08 (owner, review round 9 R9-S1-F3): the row described the 6-row cap this feature removed -->
...
| commit file list | The list of files changed by the selected commit, shown in the inspector under the commit header; collapsible, and in this feature the block the layout policy protects — it takes the height the header leaves, with no upper bound on its rows. <!-- amended 2026-09-08 (owner, review round 9 R9-S1-F3) --> |
...
| inspector | The workbench column right of or below the centre view: commit header, commit file list, optional stacked blame / file history — plus the diff viewer's parking slot, which is a real growing slot only in the Changes view. <!-- amended 2026-09-08 (owner, review round 9 R9-S1-F3): the History inspector shows no diff --> |
...
| ★ open-large gesture | Any of the explicit ways to open the diff workspace on the active file: double-click, the visible control, the shortcut (AC-06) — and, when `commitFileClickOpensWorkspace` is on, a single click as well (AC-22). The three explicit ones work in both modes. <!-- amended 2026-09-08 (owner, review round 9 R9-S1-F1) --> |
